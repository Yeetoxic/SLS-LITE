...
| `stop <server\|all> [force]` | Admin | Adapted | Local servers stop gracefully after evacuation. Both pinned `force` and additive `--force` are accepted. On ordinary servers the modifier is a safe compatibility no-op because routing is already removed before graceful shutdown and ownership remains until verified exit. On the managed lobby it requires `sls.command.stop.force`, diverts new arrivals, evacuates players to SLS-Limbo, restores primary routing if evacuation fails, suppresses recovery after a successful drain, and writes an audit log. The `all` selector remains command-branch parity work. |
...
| `status <server> [remote]` | Admin | Adapted | Local status output matched. `remote` is completed as an explicit response explaining that no daemon exists and the supervised local process state is authoritative. |
