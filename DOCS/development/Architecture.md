...
| `instance.metadata` | Versioned metadata serialization, confined atomic persistence, persistent discovery, resume validation, and definition identity checks. | `instance.model`, the read-only instance facade, and filesystem/process identity primitives. |
...
plugin metadata, and the checksum-pinned SLS-Limbo runtime. Protocol support is
inspected from the pinned NanoLimbo JAR, backend synchronization uses
ViaVersion's API, and compatibility fixtures remain under test resources rather
than the production artifact.
...
documented in [Java Extension API](../extensions/README.md). Other plugins must not depend
