...
In this branch: [SLS main compatibility](SLS_Main.md),
[SLS command compatibility](SLS_Commands.md), and
[migration from full SLS](Migration_From_SLS.md).
...
[SLS Main Compatibility Matrix](SLS_Main.md). Reviewed,
...
- Historical single-host behavior: the preserved legacy fixtures documented in
  [Migration](Migration_From_SLS.md).
...
- Velocity 4.1.0 remains a snapshot line. SLS-LITE retains the exact API build
  ...
  boundary without a compatibility review;
- ViaVersion 5.11.0 remains the optional exercised integration.

See [SLS Command Compatibility](SLS_Commands.md). Historical
single-host migration behavior is summarized in [Migration](Migration_From_SLS.md).
