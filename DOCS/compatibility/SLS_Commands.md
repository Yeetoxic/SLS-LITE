...
| `console <server> <command>` | Admin | Adapted | Safe local process input is followed by an asynchronous, cursor-isolated capture of at most eight new lines over two seconds. Live output is a separate operation under `logs`. |
| `blueprint <id>` | Admin | Supported | Pretty-prints one globally unique blueprint ID with its local launch/storage details after mixin `includes` are applied. The additive `blueprints [registry]` form remains available for catalog browsing. |
| `mixin <id>` | Admin | Supported | Pretty-prints one mixin after `extends` chains are flattened. Tab completion suggests known mixin ids. |
...
| `reload [all\|config\|blueprints\|software]` | Admin | Adapted | Every blueprint and mixin is checked: valid siblings publish together, invalid definitions are absent from the new immutable catalog, accepted/rejected blueprint counts and accepted mixin counts are reported, and exact per-file errors go to the detail log. Software/global transaction failures still reject incompatible publication. SLS-LITE-owned dynamic registrations are reconciled without mutating foreign conflicts. `config` directs the operator to restart Velocity because host-wide services cannot be rebuilt safely in place. |
