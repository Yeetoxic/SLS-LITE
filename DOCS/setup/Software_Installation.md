...
Most operators do not need to edit a software profile:

1. Accept the Minecraft EULA yourself, then enable `accept_eula` in the bundled
   Paper profile or `software.auto_accept_eula` in `config.yml`.
2. Set `server.software: paper` and an exact Minecraft version in the blueprint.
3. Join the blueprint. SLS-LITE downloads and verifies the missing Paper base
   automatically, or you can pre-download it with
   `/sls install warmup paper <version>`.
4. If installation fails, run `/sls install logs paper <version>`.

Read the model below only when creating custom profiles, pinning builds, using
manual software, or managing the cache.
