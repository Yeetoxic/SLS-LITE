...
The operator-owned `config.yml` is never rewritten automatically. Its
`config_version` is compared with the plugin's supported generation, and the
complete current example lives in the documentation instead of a duplicate
