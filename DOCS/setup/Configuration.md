...
different effects; see [Applying Changes Safely](../operations/Applying_Changes.md).
...
SLS-LITE never rewrites an existing `config.yml`. The current configuration
declares its `config_version`; startup compares that number with the supported
generation and links to the
...
| --- | --- | --- |
| `config_version` | `2` | Current positive schema generation. A configuration newer than the running plugin supports is rejected. |
...
| `security.backend_messaging.enabled` | `false` | Enables the dedicated source-verified `slslite:request` backend channel. Requires at least one source. Start at the [Backend Messaging hub](../integrations/backend-messaging/README.md), then use the [built-in guide](../integrations/backend-messaging/Built_In.md). |
...
when applicable. See the [built-in backend-messaging guide](../integrations/backend-messaging/Built_In.md)
for its schema and trust model.
