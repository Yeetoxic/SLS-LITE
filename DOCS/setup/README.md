...
In this branch: [configuration](Configuration.md),
[copyable configuration](Copyable_Config.md), [data layout](Data_Layout.md),
and [software installation](Software_Installation.md).
...
If this is your first install, ignore advanced storage choices until one server
works:

1. [Install the plugin](#install).
2. [Configure forwarding](#forwarding-and-first-connection).
3. Copy the [disposable-world recipe](../blueprints/README.md#1-disposable-world).
4. Start Velocity. Fix only the first `ACTION NEEDED` item, if one appears.
5. Run `/sls reload blueprints`, then `/sls join minigame <blueprint-id>`.

Once that works, use the [full blueprint schema](../blueprints/Schema.md) and
[storage guide](../blueprints/Volumes.md) for customization.
...
The essentials are Java 25 for this Velocity build, permission to run local
child processes, writable storage, unused loopback ports, and enough real host
memory for Velocity plus managed servers.

<details>
<summary>Complete host checklist</summary>
...
8. After accepting the Minecraft EULA, set `accept_eula: true` in each automatic
   software profile you approve, or set host-wide
   `software.auto_accept_eula: true` in `config.yml`. Both remain false by
   default.
9. Copy the [disposable-world recipe](../blueprints/README.md#1-disposable-world)
   into `blueprints/`, replace its example values, and provide its source world.
10. Start Velocity and inspect `/sls system`.

The generated defaults are safe starting points, not detected host capacity.
Before importing a real network, resize the 2048 MiB managed budget and 20-port
range for the allocation. Example blueprints are not loaded until copied or
renamed to `.yml`.
...
or the granular nodes in [Commands](../operations/Commands.md).
...
[Blueprint Recipe Book](../blueprints/README.md).
...
2. Locate the file named by `forwarding-secret-file`. Velocity or the hosting
   panel normally generates `forwarding.secret` automatically, so reuse that
   existing file and do not replace its value during SLS-LITE setup. The path is
   relative to the Velocity working directory and must not be below
   `plugins/sls-lite/`.

   Only if the configured file is genuinely absent, create it with one private,
   cryptographically random value of at least 32 characters. If a Linux shell
   ...
   With the example setting, the path is
   `<Velocity working directory>/forwarding.secret`; a host may configure a
   different relative filename. Only the account running Velocity should be
   able to read it. On a managed panel, let the host manage the generated file
   when possible. If manual creation is required, use its private file manager
...
3. In `plugins/sls-lite/config.yml`, use the matching settings:
   ...
   must exactly match `online-mode` in `velocity.toml`. Set `secret_file` to the
   same relative path as Velocity's `forwarding-secret-file`; both paths are
   resolved from the Velocity working directory and must refer to the existing
   generated secret.
...
| `Unable to read`, `empty`, or invalid forwarding secret | The regular, non-symbolic file named by `forwarding-secret-file` must exist relative to the Velocity working directory, be readable by Velocity, and match SLS-LITE's relative path. Check the file generated by Velocity or the host before creating or replacing anything. Do not reveal it while diagnosing permissions. |
...
For deeper diagnosis, use [Troubleshooting](../operations/Troubleshooting.md) without exposing
the forwarding secret.

## Updating

Before upgrading, read the release notes for the target version. For a specific
older version, use the documentation stored at that release's Git tag.
...
5. Start Velocity and review reconciliation, startup, and capability logs.
6. Run `/sls system`, `/sls info`, and a representative start/join/stop test.

Never delete persistent instance metadata to silence a compatibility error. Use the
