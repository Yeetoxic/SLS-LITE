...
See [Blueprint Volumes](Volumes.md) for transactional and filesystem
details.
...
failure instead of guessing. Otherwise, a changed instance value is published
atomically to the canonical source without creating a backup; backup and
retention policy remains under operator control.
...
[SLS main compatibility matrix](../compatibility/SLS_Main.md) for the maintained,
