...
public record BlueprintLifecyclePolicy(boolean keepAlive, Duration idleTimeout) {
  ...
  public static BlueprintLifecyclePolicy from(Blueprint blueprint, int defaultIdleShutdownSeconds) {
    ...
    boolean stopWhenEmpty = booleanAnnotation(annotations, STOP_WHEN_EMPTY, true);
    int idleSeconds =
        integerAnnotation(annotations, IDLE_SHUTDOWN_SECONDS, defaultIdleShutdownSeconds);
  ...
  private static int integerAnnotation(
      Map<String, Object> annotations, String key, int defaultValue) {
    Object value = annotation(annotations, key);
    if (value == null) {
      return defaultValue;
    }
    if (!(value instanceof Number number)
      ...
      throw new IllegalArgumentException(
          "annotation '" + NAMESPACE + "." + key + "' must be a non-negative integer");
    }
