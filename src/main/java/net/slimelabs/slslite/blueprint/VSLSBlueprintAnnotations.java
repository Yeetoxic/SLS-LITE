...
public final class VSLSBlueprintAnnotations {
  ...
  public static boolean dontStopWhenEmpty(Map<String, Object> annotations) {
    ...
    if (!(value instanceof Boolean booleanValue)) {
      throw new IllegalArgumentException("annotations.vsls.dont-stop-when-empty must be a boolean");
    }
    return booleanValue;
  }
  ...
  public static OptionalInt maxInstances(Map<String, Object> annotations) {
    return positiveInteger(
        namespace(annotations).get("max-instances"), "annotations.vsls.max-instances");
  }

  public static OptionalInt maxPlayers(Map<String, Object> annotations) {
    Map<?, ?> matchmaking = matchmaking(annotations);
    return positiveInteger(
        matchmaking.get("maxPlayers"), "annotations.vsls.matchmaking.maxPlayers");
  }
  ...
  public static List<String> onJoinCommands(Map<String, Object> annotations) {
    ...
    if (!(value instanceof List<?> entries)) {
      throw new IllegalArgumentException("annotations.vsls.on-join must be a list");
    }
    if (entries.size() > 32) {
      throw new IllegalArgumentException(
    ...
    for (int index = 0; index < entries.size(); index++) {
      ...
      if (!(configured instanceof String command) || command.isBlank()) {
        throw new IllegalArgumentException(
            "annotations.vsls.on-join[" + index + "].run must be a non-blank string");
      }
  ...
  private static Map<?, ?> matchmaking(Map<String, Object> annotations) {
    ...
    if (!(value instanceof Map<?, ?> map)) {
      throw new IllegalArgumentException("annotations.vsls.matchmaking must be an object");
    }
    return map;
  }
  ...
  private static OptionalInt positiveInteger(Object value, String path) {
    ...
    if (parsed.signum() <= 0 || parsed.compareTo(BigInteger.valueOf(Integer.MAX_VALUE)) > 0) {
      throw new IllegalArgumentException(path + " must be between 1 and " + Integer.MAX_VALUE);
    }
