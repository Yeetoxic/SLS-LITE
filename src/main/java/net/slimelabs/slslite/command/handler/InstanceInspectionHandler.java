...
final class InstanceInspectionHandler {
    ...
    void list(CommandSource source) {
        ...
        instances.getAll().stream()
                ...
                .forEach(instance -> source.sendMessage(CommandMessages.listEntry(
                        instance,
                        instanceAccess.playersOn(instance)
                )));
    ...
    void logs(CommandSource source, String[] arguments) {
        ...
        Integer page = arguments.length >= 3
                ...
                : Integer.valueOf(DEFAULT_LOG_PAGE);
        Integer lines = arguments.length == 4
                ? parseLogLines(arguments[3])
                : Integer.valueOf(DEFAULT_LOG_LINES);
        if (page == null) {
    ...
    void stats(CommandSource source, String[] arguments) {
        ...
        ManagedInstance instance = instanceAccess.resolve(
                source,
        ...
        String residentMemory = resources
                .flatMap(snapshot -> snapshot.residentBytes().isPresent()
                        ? Optional.of(formatBytes(
                                snapshot.residentBytes().getAsLong()
                        ))
                        : Optional.empty())
                .orElse("not measurable");
        String processIo = resources.map(snapshot ->
                "logical "
                        + optionalBytes(snapshot.charactersRead()) + " read / "
                        + optionalBytes(snapshot.charactersWritten()) + " written"
                        + "; storage "
                        + optionalBytes(snapshot.storageBytesRead()) + " read / "
                        + optionalBytes(snapshot.storageBytesWritten()) + " written"
        ).orElse("not measurable");
    ...
    List<String> suggestions(
        ...
        if (arguments.length == 2
                && List.of("info", "logs", "stats", "status").contains(operation)) {
