...
final class StorageStrategySelector {
    ...
    private static final List<StorageStrategy> AUTO_PRIORITY = List.of(
    ...
    StorageStrategySelection select(
        ...
        if (requested == StorageStrategy.AUTO) {
            StorageStrategy selected = AUTO_PRIORITY.stream()
        ...
        if (!implemented.contains(requested)) {
            return unavailable(
                    requested,
                    "the strategy is not implemented in this build"
            );
        }
        if (!detected.contains(requested)) {
            return unavailable(
                    requested,
                    "the required capability was not detected for this storage "
                            + "location"
            );
        }
        return new StorageStrategySelection(
    ...
    private static StorageStrategySelection unavailable(
            StorageStrategy requested,
            String detail
    ) {
        return new StorageStrategySelection(
