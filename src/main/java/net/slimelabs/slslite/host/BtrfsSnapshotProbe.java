  ...
  Result probe(Path storagePath) {
    ...
    try {
      ...
      if (!subvolumes.available()) {
        return unsupported("the btrfs userspace command is unavailable");
      }
      probe = Files.createTempDirectory(storagePath, ".sls-btrfs-probe-");
      ...
      if (!subvolumes.isSubvolume(snapshot)
          || !"source".equals(Files.readString(snapshot.resolve("marker")))) {
        return unsupported("the contained snapshot did not preserve its source");
      }
      Files.writeString(snapshot.resolve("marker"), "snapshot");
      if (!"source".equals(Files.readString(source.resolve("marker")))) {
        return unsupported("the contained snapshot write-isolation check failed");
      }
      ...
      probe = null;
      return supported(
          "contained subvolume snapshot, write isolation, deletion, " + "and cleanup passed");
    } catch (IOException | RuntimeException exception) {
      return unsupported("contained probe failed: " + message(exception));
    } finally {
