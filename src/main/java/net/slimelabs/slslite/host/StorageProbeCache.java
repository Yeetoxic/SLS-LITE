...
final class StorageProbeCache {
    ...
    private final LinkedHashMap<Identity, Map<StorageStrategy, HostCapability>>
    ...
    private void invalidateChangedIdentity(Identity current) {
        entries.keySet().removeIf(identity ->
                identity.path().equals(current.path())
    ...
    static Map<StorageStrategy, HostCapability> successfulCapabilities(
        ...
        candidates.forEach((strategy, capability) -> {
