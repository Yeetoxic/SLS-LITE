...
final class JoinPhaseTimings {
    ...
    synchronized Optional<String> complete(String outcome) {
        ...
        summary.append(" total=").append(formatMillis(elapsed(
                acceptedAt,
                completedAt
        )));
        Long queueFinishedAt = transferStartedAt != null
                ? transferStartedAt
                : readyAt != null ? readyAt : completedAt;
        if (queueTracked) {
            summary.append(" queue=").append(formatMillis(elapsed(
                    acceptedAt,
                    queueFinishedAt
            )));
        }
        if (transferStartedAt != null) {
            summary.append(" transfer=").append(formatMillis(elapsed(
                    transferStartedAt,
                    completedAt
            )));
        }
