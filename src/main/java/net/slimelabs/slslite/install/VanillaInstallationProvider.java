...
public final class VanillaInstallationProvider
    ...
    public InstallationArtifact install(
        ...
        if (metadata == null) {
            throw new SoftwareInstallationException(
                    "Unknown vanilla Minecraft version: " + version
            );
        }
        ...
        if (expectedSize <= 0 || expectedSize > MAX_DOWNLOAD_BYTES) {
            throw new SoftwareInstallationException(
                    "Vanilla artifact size is outside the allowed range: "
                            + expectedSize
            );
        }
        ...
        if (actualSize != expectedSize) {
            throw new SoftwareInstallationException(
                    "Vanilla artifact size mismatch"
            );
        }
        ...
        if (!MessageDigest.isEqual(
            ...
            throw new SoftwareInstallationException(
                    "Vanilla artifact SHA-1 mismatch"
            );
        }
    ...
    private void validateMojangUri(URI uri)
            throws SoftwareInstallationException {
        String host = uri.getHost();
        if (requireOfficialHosts
                && (!"https".equalsIgnoreCase(uri.getScheme())
                || host == null
                || !(host.equals("mojang.com")
                || host.endsWith(".mojang.com")))) {
            throw new SoftwareInstallationException(
                    "Mojang returned an untrusted download URL"
            );
        }
    }
    ...
    private static Map<?, ?> object(Object value, String name)
        ...
        throw new SoftwareInstallationException(
                "Mojang metadata is missing " + name
        );
    }
    ...
    private static String required(Map<?, ?> map, String key)
        ...
        if (value == null || value.toString().isBlank()) {
            throw new SoftwareInstallationException(
                    "Mojang metadata is missing " + key
            );
        }
    ...
    private static long copyBounded(
        ...
        try (var output = Files.newOutputStream(destination)) {
            ...
            while ((read = input.read(buffer)) >= 0) {
                ...
                if (total > maximum) {
                    throw new SoftwareInstallationException(
                            "Vanilla artifact exceeded its declared size"
                    );
                }
    ...
    private static byte[] readBounded(InputStream input, int maximum)
        ...
        while ((read = input.read(buffer)) >= 0) {
            if (output.size() + read > maximum) {
                throw new SoftwareInstallationException(
                        "Mojang metadata response is too large"
                );
            }
            output.write(buffer, 0, read);
        }
