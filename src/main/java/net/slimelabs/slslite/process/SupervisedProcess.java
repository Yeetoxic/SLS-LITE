...
public final class SupervisedProcess {
    ...
    synchronized void start() throws ProcessStartException {
        ...
        try {
            ...
            startupDeadline = owner.schedule(
                    this::handleStartupTimeout,
                    spec.startupTimeout().toMillis(),
            ...
            throw new ProcessStartException(
                    "Unable to start process for instance " + instanceId,
                    exception
            );
        }
    }
    ...
    public synchronized CompletableFuture<Integer> stop() {
        ...
        if (state == InstanceState.STARTING
            ...
            throw new IllegalStateException(
                    "Cannot stop instance " + instanceId + " while " + state
            );
        }
        ...
        try {
            ...
            outputConsumer.accept(
                    "Unable to send stop command to " + instanceId + ": "
                            + exception.getMessage()
            );
        }

        stopDeadline = owner.schedule(
                this::forceStopAfterTimeout,
    ...
    private synchronized void handleStartupTimeout() {
        ...
        TimeoutException exception = new TimeoutException(
                "Instance " + instanceId + " did not become ready within "
                        + spec.startupTimeout().toSeconds() + " seconds"
        );
        transitionToFailed();
        ready.completeExceptionally(exception);
        if (process != null && process.isAlive()) {
    ...
    private synchronized void forceStopAfterTimeout() {
        if (process != null && process.isAlive()) {
            outputConsumer.accept(
                    "Instance " + instanceId + " did not stop within "
                            + spec.stopTimeout().toSeconds() + " seconds; forcing termination"
            );
            process.destroyForcibly();
        }
    }
    ...
    private synchronized void handleExit(int exitCode) {
        ...
        if (!ready.isDone()) {
            ready.completeExceptionally(new ProcessStartException(
                    "Instance " + instanceId + " exited with code " + exitCode
