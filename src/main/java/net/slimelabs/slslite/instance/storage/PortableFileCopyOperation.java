 ...
 */
final class PortableFileCopyOperation implements InstanceDirectoryPreparer.FileCopyOperation {
  ...
  private static boolean containsSampledZeroRun(Path source, long size) throws IOException {
    ...
    try (FileChannel input = FileChannel.open(source, StandardOpenOption.READ)) {
      for (int sampleIndex = 0; sampleIndex < SAMPLE_COUNT; sampleIndex++) {
        long offset = SAMPLE_COUNT == 1 ? 0 : finalOffset * sampleIndex / (SAMPLE_COUNT - 1);
        sample.clear();
        int read = readAtMost(input, sample, offset);
        if (read == sampleLength && allZero(sample.array(), read)) {
          return true;
        }
      }
    }
  ...
  private static void copySparse(Path source, Path target, long sourceSize) throws IOException {
    ...
    try (FileChannel input = FileChannel.open(source, StandardOpenOption.READ);
      ...
      if (input.size() != sourceSize) {
        throw new IOException("Source size changed during sparse copy: " + source);
      }
  ...
  private static int readAtMost(FileChannel input, ByteBuffer target, long offset)
      throws IOException {
    int total = 0;
