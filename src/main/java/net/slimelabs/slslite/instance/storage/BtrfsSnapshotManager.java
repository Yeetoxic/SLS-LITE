...
final class BtrfsSnapshotManager {
    ...
    boolean hasManifest(Path instanceDirectory) {
        return Files.exists(
                instanceDirectory.resolve(MANIFEST_FILE),
    ...
    boolean isEligibleSource(Path sourceDirectory) throws IOException {
        ...
        return subvolumes.isSubvolume(source)
                && !subvolumes.hasNestedSubvolumes(source);
    }
    ...
    void snapshot(
        ...
        if (Files.exists(target, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException(
                    "Btrfs snapshot target already exists: " + target
            );
        }
        if (!subvolumes.isSubvolume(source)) {
            throw new IneligibleSourceException(
                    "Btrfs snapshot source is not a subvolume: " + source
            );
        }
        if (subvolumes.hasNestedSubvolumes(source)) {
            throw new IneligibleSourceException(
                    "Btrfs snapshot source contains nested subvolumes: "
                            + source
            );
        }
        ...
        if (targets.contains(target)) {
            throw new IOException(
                    "Btrfs snapshot target is already declared: " + target
            );
        }
        ...
        try {
            ...
            if (!subvolumes.isSubvolume(target)) {
                throw new IOException(
                        "Btrfs snapshot command did not create a subvolume: "
                                + target
                );
            }
                ...
                if (!subvolumes.isSubvolume(target)) {
                    throw new IOException(
                            "Refusing to traverse a declared Btrfs snapshot "
                                    + "that is not a subvolume: " + target
                    );
                }
                subvolumes.delete(target);
    ...
    private List<Path> readManifest(Path instance) throws IOException {
        ...
        if (!Files.isRegularFile(manifest, LinkOption.NOFOLLOW_LINKS)
            ...
            throw new IOException(
                    "Btrfs manifest is missing, unsafe, or too large: "
                            + manifest
            );
        }
        ...
        if (!MANIFEST_VERSION.equals(properties.getProperty("version"))) {
            throw new IOException(
                    "Unsupported Btrfs manifest version: " + manifest
            );
        }
        int count = boundedCount(properties.getProperty("subvolumes"));
        if (count == 0) {
            throw new IOException(
                    "Btrfs manifest declares no subvolumes: " + manifest
            );
        }
        ...
        for (int index = 0; index < count; index++) {
            ...
            if (value == null || value.isBlank()) {
                throw new IOException(
                        "Missing Btrfs manifest subvolume " + index
                );
            }
    ...
    private void writeOrDeleteManifest(
            Path instance,
            List<Path> targets
    ) throws IOException {
        if (targets.isEmpty()) {
    ...
    private void writeManifest(Path instance, List<Path> targets)
            throws IOException {
        Properties properties = new Properties();
        ...
        for (int index = 0; index < targets.size(); index++) {
            properties.setProperty(
                    "subvolume." + index,
                    portableRelative(instance.relativize(targets.get(index)))
            );
        }
        Path manifest = instance.resolve(MANIFEST_FILE);
    ...
    private Path checkedInstance(Path value) throws IOException {
        ...
        if (instance.equals(instancesRoot)
            ...
            throw new IOException(
                    "Btrfs instance is missing or outside " + instancesRoot
            );
        }
        ...
        if (realInstance.equals(realRoot)
            ...
            throw new IOException(
                    "Btrfs instance resolves outside " + realRoot
            );
        }
        return instance;
    }
    ...
    private Path checkedSource(Path value) throws IOException {
        ...
        if (!Files.isDirectory(source, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException(
                    "Btrfs snapshot source is missing or unsafe: " + source
            );
        }
        ...
        if (realSource.equals(realContent)
            ...
            throw new IOException(
                    "Btrfs snapshot source must stay in managed content: "
                            + source
            );
        }
        return realSource;
    ...
    private static Path checkedTarget(Path instance, Path relative)
        ...
        if (relative.isAbsolute() || relative.toString().isBlank()) {
            throw new IOException(
                    "Btrfs snapshot target must be a relative instance path"
            );
        }
        Path target = instance.resolve(relative).normalize();
        if (target.equals(instance) || !target.startsWith(instance)) {
            throw new IOException(
                    "Btrfs snapshot target escapes the instance: " + relative
            );
        }
        ...
        for (Path segment : instance.relativize(target)) {
            ...
            if (Files.isSymbolicLink(current)) {
                throw new IOException(
                        "Btrfs snapshot paths must not contain symbolic links: "
                                + current
                );
            }
        }
    ...
    private static void rejectOverlappingTarget(
        ...
        for (Path previous : existing) {
            if (target.startsWith(previous) || previous.startsWith(target)) {
                throw new IOException(
                        "Btrfs snapshot targets overlap: "
                                + previous + " and " + target
                );
            }
        }
    ...
    private static final class CommandSubvolumeAdapter
            implements SubvolumeAdapter {
        ...
        private static boolean hasSubvolumeRootInode(Path path)
                throws IOException {
            try {
                Object inode = Files.getAttribute(
                        path,
                        "unix:ino",
                        LinkOption.NOFOLLOW_LINKS
                );
                return inode instanceof Number number
                        && number.longValue() == 256L;
            ...
            Files.walkFileTree(path, new java.nio.file.SimpleFileVisitor<>() {
                ...
                public java.nio.file.FileVisitResult preVisitDirectory(
                    ...
                    if (!directory.equals(path)
                            && hasSubvolumeRootInode(directory)) {
                        nested[0] = true;
        ...
        private static void requireSuccess(List<String> command)
                throws IOException {
            int result = execute(command);
            if (result != 0) {
                throw new IOException(
                        String.join(" ", command.subList(0, 3))
