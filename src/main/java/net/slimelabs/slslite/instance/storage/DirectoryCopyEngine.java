...
final class DirectoryCopyEngine {
  ...
  private static final long[] RETRY_DELAYS_MILLIS = {250, 750, 2_000, 5_000};
  ...
  DirectoryCopyEngine(
    ...
    this.retrySleeper = java.util.Objects.requireNonNull(retrySleeper, "retrySleeper");
    if (parallelism < 1) {
      throw new IllegalArgumentException("Copy parallelism must be positive");
    }
    this.parallelism = parallelism;
  }

  void validateSource(Path source, BooleanSupplier cancellationRequested) throws IOException {
    Files.walkFileTree(
        source,
  ...
  void copyDirectory(Path source, Path destination, BooleanSupplier cancellationRequested)
      throws IOException {
    if (parallelism == 1) {
      copyDirectorySequentially(source, destination, cancellationRequested);
      return;
    }
    int maximumInFlight = parallelism * 2;
    try (BoundedCopyBatch batch = new BoundedCopyBatch(parallelism, maximumInFlight)) {
      Files.walkFileTree(
          source,
          new SimpleFileVisitor<>() {
            @Override
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
              ...
              batch.submit(() -> copyFileWithRetry(file, target, cancellationRequested));
              return FileVisitResult.CONTINUE;
            }
          });
      batch.complete();
  ...
  void copyDirectoryReplacing(Path source, Path destination, BooleanSupplier cancellationRequested)
      throws IOException {
    Files.walkFileTree(
        source,
        new SimpleFileVisitor<>() {
  ...
  void mergeDirectoryFirstWins(Path source, Path destination, BooleanSupplier cancellationRequested)
      throws IOException {
    Files.walkFileTree(
        source,
        new SimpleFileVisitor<>() {
  ...
  void copyFile(Path source, Path target, BooleanSupplier cancellationRequested)
      throws IOException {
    copyFileWithRetry(source, target, cancellationRequested);
  }

  private void copyDirectorySequentially(
      Path source, Path destination, BooleanSupplier cancellationRequested) throws IOException {
    Files.walkFileTree(
        source,
        new SimpleFileVisitor<>() {
          @Override
  ...
  private void copyFileWithRetry(Path source, Path target, BooleanSupplier cancellationRequested)
      throws IOException {
    ...
    for (int attempt = 0; attempt <= RETRY_DELAYS_MILLIS.length; attempt++) {
      ...
      try {
        ...
        if (attempt == RETRY_DELAYS_MILLIS.length || !isRetryableCopyFailure(exception)) {
          throw exception;
        }
        try {
          Files.deleteIfExists(target);
        ...
        try {
          sleepWithCancellation(RETRY_DELAYS_MILLIS[attempt], cancellationRequested);
        } catch (InterruptedException interrupted) {
          Thread.currentThread().interrupt();
          IOException failure =
              new IOException("Interrupted while retrying file copy: " + source, interrupted);
  ...
  private void sleepWithCancellation(long milliseconds, BooleanSupplier cancellationRequested)
  ...
  private static void rejectSymbolicLink(Path path, BasicFileAttributes attrs) throws IOException {
