...
final class PersistentInstanceTransaction {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(PersistentInstanceTransaction.class);
  ...
  PersistentInstanceTransaction(Path instancesRoot, PreparedStorageLifecycle storageLifecycle) {
    this(instancesRoot, storageLifecycle, UUID::randomUUID);
  }

  PersistentInstanceTransaction(
      Path instancesRoot, PreparedStorageLifecycle storageLifecycle, Supplier<UUID> nonceSupplier) {
    this.instancesRoot = instancesRoot.toAbsolutePath().normalize();
    this.storageLifecycle = java.util.Objects.requireNonNull(storageLifecycle, "storageLifecycle");
  ...
  void replace(
    ...
    java.util.Objects.requireNonNull(stagingPreparer, "stagingPreparer");
    ...
    try {
      ...
      throw new InstancePreparationException(
          "Unable to reset persistent instance " + destination, exception);
    }
  }
  ...
  int recover(DirectoryCommitVerifier verifier) throws IOException {
    ...
    try (var entries = Files.list(instancesRoot)) {
      directories =
          entries.filter(path -> Files.isDirectory(path, LinkOption.NOFOLLOW_LINKS)).toList();
    }
    ...
    for (Path backup : directories) {
      ...
      Path staging = transactionDirectory(instanceId, "reset", match.group(2));
    ...
    for (Path staging : directories) {
      ...
      Matcher match = STAGING_DIRECTORY.matcher(staging.getFileName().toString());
      if (!match.matches() || !InstanceIdGenerator.isValid(match.group(1))) {
        continue;
      }
      storageLifecycle.delete(staging);
  ...
  private Path transactionDirectory(String instanceId, String phase, String nonce) {
    return instancesRoot.resolve("." + instanceId + "." + phase + "-" + nonce);
  }
