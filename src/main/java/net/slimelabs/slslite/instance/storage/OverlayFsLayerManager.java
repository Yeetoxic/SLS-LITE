...
final class OverlayFsLayerManager {
    ...
    OverlayFsLayerManager(Path instancesRoot, Path contentRoot) {
        this(
                instancesRoot,
                contentRoot,
                new CommandMountAdapter(new OverlayFsMountOperations())
        );
    }

    OverlayFsLayerManager(
            Path instancesRoot,
            Path contentRoot,
            MountAdapter mounts
    ) {
    ...
    void prepare(Path instanceDirectory, List<Layer> layers) throws IOException {
        ...
        if (Files.exists(manifest, LinkOption.NOFOLLOW_LINKS)
            ...
            throw new IOException(
                    "OverlayFS metadata collides with existing instance content: "
                            + instance
            );
        }
    ...
    boolean hasManifest(Path instanceDirectory) {
        ...
        return instance.startsWith(instancesRoot)
                && Files.isRegularFile(
                        instance.resolve(MANIFEST_FILE),
    ...
    void assertNoMountsBeneath(Path instanceDirectory) throws IOException {
        ...
        if (!remaining.isEmpty()) {
            throw new IOException(
                    "Refusing to traverse instance while mount remains at "
                            + remaining.get(0)
            );
        }
    }

    private List<ResolvedLayer> resolveNewLayers(
            Path instance,
            List<Layer> layers
    ) throws IOException {
        ...
        for (int index = 0; index < layers.size(); index++) {
            ...
            for (Path previous : targets) {
                if (target.startsWith(previous) || previous.startsWith(target)) {
                    throw new IOException(
                            "OverlayFS targets overlap: " + previous + " and " + target
                    );
                }
            }
            targets.add(target);
            if (Files.exists(target, LinkOption.NOFOLLOW_LINKS)) {
                throw new IOException(
                        "OverlayFS target collides with existing content: " + target
                );
            }
            ...
            Path layerDirectory = instance.resolve(LAYERS_DIRECTORY)
    ...
    private List<ResolvedLayer> readManifest(Path instance) throws IOException {
        ...
        if (!MANIFEST_VERSION.equals(version)
                && !LEGACY_MANIFEST_VERSION.equals(version)) {
            throw new IOException("Unsupported OverlayFS manifest version: " + manifest);
        }
        String recordedAdapter = LEGACY_MANIFEST_VERSION.equals(version)
                ? "overlay"
                : required(properties, "mount-adapter");
        if (!mounts.storageType().equals(recordedAdapter)) {
            throw new IOException(
                    "OverlayFS instance requires mount adapter "
                            + recordedAdapter + " but " + mounts.storageType()
                            + " is configured"
            );
        }
        ...
        if (!Files.isDirectory(layerRoot, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException(
                    "OverlayFS layer root is missing or unsafe: " + layerRoot
            );
        }
        List<ResolvedLayer> resolved = new ArrayList<>();
        List<Path> targets = new ArrayList<>();
        for (int index = 0; index < count; index++) {
            ...
            targets.add(target);
            int lowerCount = boundedInteger(
                    properties,
                    prefix + "lowers",
                    MAX_LOWERS_PER_LAYER
            );
            if (lowerCount == 0) {
            ...
            for (int lower = 0; lower < lowerCount; lower++) {
                lowers.add(checkedLower(Path.of(required(
                        properties,
                        prefix + "lower." + lower
                ))));
            }
            Path layerDirectory = instance.resolve(LAYERS_DIRECTORY)
                    .resolve(Integer.toString(index))
                    .normalize();
            Path upper = checkedLayerDirectory(
                    instance,
                    layerDirectory.resolve("upper")
            );
            Path work = checkedLayerDirectory(
                    instance,
                    layerDirectory.resolve("work")
            );
            if (!Files.isDirectory(target, LinkOption.NOFOLLOW_LINKS)
                ...
                throw new IOException(
                        "OverlayFS manifest references missing layer directories"
                );
            }
            resolved.add(new ResolvedLayer(
                    target,
        ...
        try {
            for (ResolvedLayer layer : layers) {
                if (mounts.isMounted(layer.target())) {
                    throw new IOException(
                            "OverlayFS target is unexpectedly already mounted: "
                                    + layer.target()
                    );
                }
    ...
    private void writeManifest(
        ...
        for (int index = 0; index < layers.size(); index++) {
            ...
            properties.setProperty(
                    prefix + "target",
                    portableRelative(instance.relativize(layer.target()))
            );
            properties.setProperty(
                    prefix + "lowers",
                    Integer.toString(layer.lowerDirectories().size())
            );
            for (int lower = 0; lower < layer.lowerDirectories().size(); lower++) {
                properties.setProperty(
                        prefix + "lower." + lower,
                        layer.lowerDirectories().get(lower).toString()
                );
            }
        }
        Path manifest = instance.resolve(MANIFEST_FILE);
    ...
    private Path checkedInstance(Path value) throws IOException {
        ...
        if (instance.equals(instancesRoot) || !instance.startsWith(instancesRoot)) {
            throw new IOException(
                    "OverlayFS instance must stay below " + instancesRoot
            );
        }
        if (!Files.isDirectory(instance, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException(
                    "OverlayFS instance is missing or unsafe: " + instance
            );
        }
        ...
        if (realInstance.equals(realRoot) || !realInstance.startsWith(realRoot)) {
            throw new IOException(
                    "OverlayFS instance resolves outside " + realRoot
            );
        }
        return instance;
    }
    ...
    private Path checkedLower(Path value) throws IOException {
        ...
        if (realLower.equals(realContent) || !realLower.startsWith(realContent)
                || realLower.startsWith(realInstances)) {
            throw new IOException(
                    "OverlayFS lower directory must stay in managed content: " + lower
            );
        }
        return realLower;
    }

    private static Path checkedLayerDirectory(Path instance, Path value)
            throws IOException {
        ...
        if (!directory.startsWith(layerRoot) || directory.equals(layerRoot)) {
            throw new IOException(
                    "OverlayFS layer directory escapes managed metadata"
            );
        }
        rejectSymbolicSegments(instance, directory);
        if (!Files.isDirectory(directory, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException(
                    "OverlayFS layer directory is missing or unsafe: " + directory
            );
        }
        Path realLayerRoot = layerRoot.toRealPath();
        Path realDirectory = directory.toRealPath();
        if (realDirectory.equals(realLayerRoot)
                || !realDirectory.startsWith(realLayerRoot)) {
            throw new IOException(
                    "OverlayFS layer directory resolves outside managed metadata"
            );
        }
        return directory;
    }

    private static void rejectOverlappingTarget(
            List<Path> existing,
            Path target
    ) throws IOException {
    ...
    private static void rejectSymbolicSegments(Path root, Path target)
            throws IOException {
        Path current = root;
        for (Path segment : root.relativize(target)) {
            ...
            if (Files.isSymbolicLink(current)) {
                throw new IOException(
                        "OverlayFS paths must not contain symbolic links: " + current
                );
            }
        }
    }
        ...
        Layer {
            ...
            if (lowerDirectories.isEmpty()) {
                throw new IllegalArgumentException(
                        "OverlayFS layer requires at least one lower directory"
                );
            }
        }
    }
    ...
    private record CommandMountAdapter(OverlayFsMountOperations operations)
            implements MountAdapter {
