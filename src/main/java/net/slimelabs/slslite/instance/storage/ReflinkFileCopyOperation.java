...
final class ReflinkFileCopyOperation
        implements InstanceDirectoryPreparer.FileCopyOperation {

    private static final Logger LOGGER =
            LoggerFactory.getLogger(ReflinkFileCopyOperation.class);
        ...
        try {
            ...
            if (!Files.isRegularFile(target)) {
                throw new IOException(
                        "Reflink operation did not create a regular file: " + target
                );
            }
            ...
            if (!portableFallbackAllowed) {
                throw new IOException(
                        "Required reflink clone failed for " + source + ": "
                                + message(exception),
                        exception
                );
            }
        ...
        public void clone(Path source, Path target) throws IOException {
            ...
            try {
                ...
                if (process.exitValue() != 0) {
                    throw new IOException(
                            "cp --reflink=always exited with code "
                                    + process.exitValue()
                    );
                }
                ...
                throw new IOException(
                        "cp --reflink=always was interrupted",
                        exception
                );
            } finally {
