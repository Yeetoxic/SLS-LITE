...
final class SnapshotHookLayerManager {
    ...
    static final String MANIFEST_FILE =
            ".sls-lite-snapshot-hook.properties";
    ...
    SnapshotHookLayerManager(
            Path instancesRoot,
            Path contentRoot,
            SnapshotHookClient client
    ) {
        this(
                instancesRoot,
                contentRoot,
                new ClientHookAdapter(client)
        );
    }
    ...
    void prepare(Path instanceValue, Path sourceValue, Path relativeTarget)
            throws IOException {
        ...
        if (Files.exists(target, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException(
                    "Snapshot-hook target already exists: " + target
            );
        }
        ...
        try {
            ...
            if (!Files.isDirectory(target, LinkOption.NOFOLLOW_LINKS)) {
                throw new IOException(
                        "Snapshot helper did not materialize target: " + target
                );
            }
        ...
        try {
            for (Layer layer : readManifest(instance)) {
                ...
                if (!Files.isDirectory(
                    ...
                    throw new IOException(
                            "Snapshot helper did not resume target: "
                                    + layer.target()
                    );
                }
                resumed.add(layer);
    ...
    void delete(Path instanceValue) throws IOException {
        Path instance = checkedInstance(instanceValue);
        List<Layer> layers = new ArrayList<>(readManifest(instance));
        layers.sort(Comparator.comparingInt(
                layer -> -layer.target().getNameCount()
        ));
        IOException failure = null;
        for (Layer layer : layers) {
            try {
                ...
                if (Files.exists(
                    ...
                    throw new IOException(
                            "Snapshot helper delete left its target present: "
                                    + layer.target()
                    );
                }
            } catch (IOException exception) {
    ...
    private void assertNoMounts(Path target) throws IOException {
        List<Path> mounts = new OverlayFsMountOperations()
                .mountPointsBeneath(target);
        if (!mounts.isEmpty()) {
            throw new IOException(
                    "Snapshot helper left managed mounts beneath "
                            + target + ": " + mounts
            );
        }
    }
    ...
    private List<Layer> readManifest(Path instance) throws IOException {
        ...
        if (!Files.isRegularFile(manifest, LinkOption.NOFOLLOW_LINKS)
            ...
            throw new IOException(
                    "Snapshot-hook manifest is unsafe: " + manifest
            );
        }
        ...
        if (!MANIFEST_VERSION.equals(values.getProperty("version"))) {
            throw new IOException(
                    "Unsupported snapshot-hook manifest: " + manifest
            );
        }
        int count = count(values.getProperty("layers"));
        if (count == 0) {
            throw new IOException(
                    "Snapshot-hook manifest declares no layers"
            );
        }
    ...
    private void writeOrDeleteManifest(Path instance, List<Layer> layers)
            throws IOException {
        if (layers.isEmpty()) {
    ...
    private void writeManifest(Path instance, List<Layer> layers)
            throws IOException {
        Properties values = new Properties();
        ...
        for (int index = 0; index < layers.size(); index++) {
            ...
            values.setProperty(
                    "layer." + index + ".source",
                    layer.source().toString()
            );
            values.setProperty(
                    "layer." + index + ".target",
                    portable(instance.relativize(layer.target()))
            );
        }
        Path manifest = instance.resolve(MANIFEST_FILE);
    ...
    private Path checkedInstance(Path value) throws IOException {
        ...
        if (instance.equals(instancesRoot)
            ...
            throw new IOException(
                    "Snapshot-hook instance is missing or outside "
                            + instancesRoot
            );
        }
        Path realRoot = instancesRoot.toRealPath();
        Path realInstance = instance.toRealPath();
        if (realInstance.equals(realRoot)
                || !realInstance.startsWith(realRoot)) {
            throw new IOException(
                    "Snapshot-hook instance resolves outside " + realRoot
            );
        }
        return instance;
    }
    ...
    private Path checkedSource(Path value) throws IOException {
        ...
        if (!Files.isDirectory(source, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException(
                    "Snapshot-hook source is missing: " + source
            );
        }
        ...
        if (realSource.equals(realContent)
            ...
            throw new IOException(
                    "Snapshot-hook source must stay in managed content: "
                            + source
            );
        }
        return realSource;
    ...
    private static Path checkedTarget(Path instance, Path relative)
        ...
        for (Path segment : instance.relativize(target)) {
            ...
            if (Files.isSymbolicLink(current)) {
                throw new IOException(
                        "Snapshot-hook paths must not contain symbolic links"
                );
            }
        }
        return target;
    ...
    private static void rejectOverlap(List<Layer> layers, Path target)
            throws IOException {
        for (Layer layer : layers) {
            if (target.startsWith(layer.target())
                    || layer.target().startsWith(target)) {
                throw new IOException(
                        "Snapshot-hook targets overlap: "
                                + layer.target() + " and " + target
                );
            }
        }
    }
    ...
    private static int count(String value) throws IOException {
        try {
            ...
            throw new IOException(
                    "Invalid snapshot-hook manifest layer count"
            );
        }
    }
    ...
    private static String required(Properties values, String key)
        ...
        if (value == null || value.isBlank()) {
            throw new IOException(
                    "Missing snapshot-hook manifest value " + key
            );
        }
        return value;
    }
    ...
    private record ClientHookAdapter(SnapshotHookClient client)
