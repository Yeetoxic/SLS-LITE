...
final class BoundedCopyBatch implements AutoCloseable {
  ...
  BoundedCopyBatch(int parallelism, int maximumInFlight) {
    if (parallelism < 1 || maximumInFlight < parallelism) {
      throw new IllegalArgumentException(
          "Copy parallelism must be positive and no greater than " + "the in-flight bound");
    }
    executor =
        Executors.newFixedThreadPool(
            parallelism, Thread.ofPlatform().name("sls-file-copy-", 0).daemon(true).factory());
  ...
  public void close() throws IOException {
    ...
    if (accepting.compareAndSet(true, false)) {
      firstFailure.compareAndSet(null, new IOException("Copy batch was aborted before completion"));
    }
    drain();
    Throwable failure = firstFailure.get();
    if (failure != null) {
      throw new IOException("Copy batch cleanup observed a failure", failure);
    }
  }
  ...
  private void acquireCapacity() throws IOException {
    try {
      ...
      IOException failure =
          new IOException("Interrupted while waiting for bounded copy capacity", exception);
      firstFailure.compareAndSet(null, failure);
      throw failure;
    }
  }
    ...
    while (!tasks.isTerminated()) {
      try {
        ...
        firstFailure.compareAndSet(
            null, new IOException("Interrupted while draining copy work", exception));
      }
    }
    if (interrupted) {
      Thread.currentThread().interrupt();
  ...
  private void ensureAccepting() {
    if (!accepting.get()) {
      throw new IllegalStateException("Copy batch no longer accepts work");
    }
  }
