...
final class SoftwareBaseDirectoryResolver {
    ...
    SoftwareBaseDirectoryResolver(
        ...
        this(
                paths,
                installations == null ? null : installations::ensureInstalled
        );
    }

    SoftwareBaseDirectoryResolver(
            JavaJarProcessSpecFactory paths,
            InstallationRequest installations
    ) {
    ...
    Path resolve(
        ...
        CompletableFuture<Path> installation =
                installations.ensureInstalled(profile, version);
        try {
            while (true) {
                if (cancellationRequested.getAsBoolean()) {
                    throw new ProcessSpecificationException(
                ...
                try {
                    return installation.get(
                            INSTALLATION_POLL_MILLISECONDS,
    ...
    interface InstallationRequest {
        CompletableFuture<Path> ensureInstalled(
                SoftwareProfile profile,
                String version
        );
    }
