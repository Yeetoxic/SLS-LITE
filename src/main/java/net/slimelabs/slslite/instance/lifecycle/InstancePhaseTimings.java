...
public final class InstancePhaseTimings {
    ...
    private static final Phase[] TERMINATION_PHASES = {
            Phase.SHUTDOWN,
            Phase.CLEANUP
    };
    ...
    public synchronized void finish(Phase phase) {
        ...
        if (started != null) {
            elapsedNanos.put(
                    phase,
                    nonNegativeElapsed(started, nanoTime.getAsLong())
            );
        }
    }
    ...
    public synchronized Optional<String> provisioningSummary(String outcome) {
        ...
        long completedAt = provisionedAt == null
                ? nanoTime.getAsLong()
                : provisionedAt;
        return Optional.of(summary(
                outcome,
                nonNegativeElapsed(acceptedAt, completedAt),
                PROVISIONING_PHASES
        ));
    }
    ...
    public synchronized Optional<String> terminationSummary(String outcome) {
        ...
        return Optional.of(summary(
                outcome,
                nonNegativeElapsed(acceptedAt, nanoTime.getAsLong()),
                TERMINATION_PHASES
        ));
    }
