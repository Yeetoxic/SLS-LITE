...
public final class TextFileConfigEditor {
  ...
  public static void apply(Path instanceDirectory, Map<String, Map<String, String>> patches)
      throws IOException {
  ...
  private static void apply(Path root, String configuredTarget, Map<String, String> replacements)
      throws IOException {
    ...
    try {
      ...
      if (Files.size(temporary) > MAX_FILE_BYTES) {
        throw new IOException(
            "Patched text config exceeds " + MAX_FILE_BYTES + " bytes: " + target);
      }
  ...
  private static void validatePrefixes(Map<String, String> replacements) throws IOException {
    java.util.List<String> prefixes = java.util.List.copyOf(replacements.keySet());
    for (int left = 0; left < prefixes.size(); left++) {
      for (int right = left + 1; right < prefixes.size(); right++) {
        ...
        if (first.startsWith(second) || second.startsWith(first)) {
          throw new IOException(
              "Text config replacement prefixes overlap: '" + first + "' and '" + second + "'");
        }
      }
    }
  }
