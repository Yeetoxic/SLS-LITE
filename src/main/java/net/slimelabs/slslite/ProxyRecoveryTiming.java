  ...
  synchronized Optional<String> complete(String outcome, String primary) {
    ...
    String normalizedOutcome =
        outcome == null || outcome.isBlank()
            ? "unknown"
            : outcome.strip().replace(' ', '-').toLowerCase(Locale.ROOT);
    String normalizedPrimary =
        primary == null || primary.isBlank() ? "unavailable" : primary.strip();
    return Optional.of(
        "outcome="
            + normalizedOutcome
            + " total="
            + String.format(Locale.ROOT, "%.3fms", elapsed / 1_000_000.0d)
            + " primary="
            + normalizedPrimary);
  }
}
