...
public final class SLSLimboProvider implements LobbyProvider {
  ...
  private static final Pattern READY_PATTERN = Pattern.compile("Server started on ");
  ...
  public void start() {
    synchronized (this) {
      ...
      started = true;
      if (!config.enabled()) {
        ready.completeExceptionally(new IllegalStateException("SLS-Limbo is disabled"));
        return;
      }
  ...
  public CompletableFuture<Void> evacuate(String serverName) {
    ...
    return CompletableFuture.failedFuture(
        new IllegalStateException("SLS-Limbo evacuation must use the fallback coordinator"));
  }
  ...
  public void close() {
    ...
    synchronized (this) {
      ...
      current = process;
      if (!ready.isDone()) {
        ready.completeExceptionally(new CancellationException("SLS-Limbo is shutting down"));
      }
    }
    ...
    if (!memoryReserved) {
      if (!resourceBudget.tryReserve(SERVER_NAME, config.memoryMiB())) {
        throw new IllegalStateException(
            "Insufficient managed memory for SLS-Limbo: " + config.memoryMiB() + " MiB");
      }
      memoryReserved = true;
    }
    if (port < 0) {
      ...
      launched
          ...
          .whenComplete(
              (exitCode, failure) -> {
                ...
                Throwable cause =
                    failure == null
                        ? new IllegalStateException("SLS-Limbo exited with code " + exitCode)
                        : failure;
  ...
  private void publishReady(SupervisedProcess launched, long attemptGeneration) {
      ...
      synchronized (this) {
        ...
        registeredServer =
            proxy
                ...
                .orElseThrow(
                    () -> new IllegalStateException("SLS-Limbo was not registered with Velocity"));
  ...
  private void handleFailure(
      SupervisedProcess failedProcess, long attemptGeneration, Throwable failure) {
    int nextAttempt;
    long delay;
    boolean exhausted;
    synchronized (this) {
      ...
      if (exhausted) {
        ...
        try {
          retryTask =
              recoveryScheduler.schedule(() -> prepareAndLaunch(true), delay, TimeUnit.SECONDS);
    ...
    if (exhausted) {
      ...
      logger.warn(
          "SLS-Limbo unavailable; recovery attempt {}/{} starts " + "in {} second(s): {}",
          nextAttempt,
          recoveryPolicy.maxAttempts(),
          delay,
          rootMessage(failure));
  ...
  private void markStable(SupervisedProcess stableProcess, long attemptGeneration) {
    synchronized (this) {
      if (closed
          || generation != attemptGeneration
  ...
  private void releaseAfterTerminalFailure(SupervisedProcess failedProcess) {
    ...
    failedProcess.exitFuture().whenComplete((exitCode, exitFailure) -> releaseResources());
  }

  private ProcessSpec processSpec(SLSLimboInstaller.SLSLimboInstallation currentInstallation) {
