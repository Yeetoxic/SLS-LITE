    ...
    public static void main(String[] arguments) throws Exception {
        ...
        StorageStrategy selected = check.selection()
                ...
                .orElseThrow(() -> new IllegalStateException(
                        "Automatic storage selection was unavailable"
                ));
        if (selected != StorageStrategy.OVERLAY) {
            throw new IllegalStateException(
                    "Expected OverlayFS on tmpfs after its contained probe, got "
                            + selected.configValue()
            );
        }
        HostCapability overlay = check.capabilities().stream()
                .filter(capability ->
        ...
        if (overlay.status() != HostCapabilityStatus.PASS) {
            throw new IllegalStateException(
                    "OverlayFS capability did not pass: " + overlay.detail()
            );
        }
