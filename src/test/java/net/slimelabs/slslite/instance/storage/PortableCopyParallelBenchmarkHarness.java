...
public final class PortableCopyParallelBenchmarkHarness {
  ...
  private static Result benchmark(Path root, String name, Path source) throws Exception {
    List<Path> files;
    ...
    InstanceDirectoryPreparer parallel =
        new InstanceDirectoryPreparer(root.resolve(name + "-parallel"), root);
  ...
  private static void verify(List<Path> sources, Path target) throws Exception {
    for (Path source : sources) {
      if (Files.mismatch(source, target.resolve(source.getFileName())) != -1) {
        throw new IllegalStateException("Copied content differs for " + source);
      }
    }
  }

  private static Path createWorkload(Path directory, int fileCount, byte[] content)
      throws Exception {
    Files.createDirectory(directory);
    for (int index = 0; index < fileCount; index++) {
      Files.write(directory.resolve(String.format("%05d.bin", index)), content);
    }
    return directory;
  }
