...
public final class OverlayFsRealKernelHarness {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    OverlayFsLayerManager manager = new OverlayFsLayerManager(instances, content);
    Path target = instance.resolve("world");
    try {
      manager.prepare(
          instance,
          List.of(new OverlayFsLayerManager.Layer(Path.of("world"), List.of(first, second))));
      ...
      if (!Files.isRegularFile(instance.resolve(".sls-lite-overlay-layers/0/upper/new.dat"))) {
        throw new IllegalStateException("Private upper-layer file did not survive unmount");
      }
      ...
      try {
        manager.suspend(instance);
        throw new IllegalStateException("Manager unmounted an unexpected filesystem");
      } catch (java.io.IOException expected) {
        if (!new OverlayFsMountOperations().isMounted(target)) {
          throw new IllegalStateException("Unexpected filesystem was not preserved", expected);
        }
      } finally {
  ...
  private static void run(String... command) throws Exception {
    ...
    if (process.waitFor() != 0) {
      throw new IllegalStateException(command[0] + " failed with exit code " + process.exitValue());
    }
  }
}
