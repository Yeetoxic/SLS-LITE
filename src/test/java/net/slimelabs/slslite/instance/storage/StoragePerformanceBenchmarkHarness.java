...
public final class StoragePerformanceBenchmarkHarness {
  ...
  public static void main(String[] arguments) throws Exception {
    if (arguments.length < 3 || arguments.length > 6 || arguments.length == 5) {
    ...
    int repeats = arguments.length == 4 ? Integer.parseInt(arguments[3]) : 3;
    ...
    if (repeats < 1 || repeats > MAX_REPEATS) {
      throw new IllegalArgumentException("Repeat count must be between 1 and " + MAX_REPEATS);
    }
    if (!Files.isDirectory(source, LinkOption.NOFOLLOW_LINKS)) {
      throw new IllegalArgumentException("Source must be an existing directory: " + source);
    }
    if (!Files.isDirectory(root, LinkOption.NOFOLLOW_LINKS) || !isEmpty(root)) {
      throw new IllegalArgumentException(
          "Benchmark root must be an existing empty directory: " + root);
    }
    if (root.startsWith(source) || source.startsWith(root)) {
      throw new IllegalArgumentException("Source and benchmark root must not contain one another");
    }
    ...
    for (int run = 1; run <= repeats; run++) {
      String instanceId = "benchmark." + String.format(Locale.ROOT, "%06d", run);
      ...
      try (RssSampler rss = new RssSampler()) {
        ...
        if (Files.exists(target, LinkOption.NOFOLLOW_LINKS)) {
          throw new IllegalStateException("Cleanup left benchmark target present: " + target);
        }
        samples.add(
            new Sample(
                run,
    ...
    printHeader(profile, source, repeats, sourceSize, sourceAllocated);
  ...
  private static void printHeader(
      String profile, Path source, int repeats, TreeSize size, OptionalLong allocated) {
    System.out.printf(
        Locale.ROOT,
  ...
  private static Summary printSummary(String profile, List<Sample> samples) {
    List<Long> preparation = samples.stream().map(Sample::preparationNanos).sorted().toList();
    List<Long> cleanup = samples.stream().map(Sample::cleanupNanos).sorted().toList();
    Summary summary =
        new Summary(
  ...
  private static void enforceThresholds(String profile, Summary summary, Thresholds thresholds) {
        ...
        new SimpleFileVisitor<>() {
          ...
          public FileVisitResult visitFile(Path file, BasicFileAttributes attributes)
            ...
            if (!attributes.isRegularFile()) {
              throw new IOException("Benchmark source contains a non-regular entry: " + file);
            }
  ...
  private static OptionalLong allocatedBytes(Path path) {
    ...
    try {
      process =
          new ProcessBuilder("du", "-s", "-B1", "--", path.toString())
      ...
      String value = new String(output, StandardCharsets.UTF_8).strip().split("\\s+", 2)[0];
  ...
  private record Thresholds(double preparationP95Millis, double cleanupP95Millis) {}
  ...
  private record IoSnapshot(
      OptionalLong rchar, OptionalLong wchar, OptionalLong readBytes, OptionalLong writeBytes) {
            ...
            Files.readAllLines(path).stream()
                ...
                .collect(
                    java.util.stream.Collectors.toMap(
                        fields -> fields[0].strip(), fields -> Long.parseLong(fields[1].strip())));
    ...
    private static OptionalLong difference(OptionalLong later, OptionalLong earlier) {
      if (later.isEmpty() || earlier.isEmpty()) {
        return OptionalLong.empty();
      }
      return OptionalLong.of(Math.max(0L, later.getAsLong() - earlier.getAsLong()));
    }
  }
  ...
  private static final class RssSampler implements AutoCloseable {
    ...
    private void sample() {
      currentRssBytes().ifPresent(value -> peakBytes.accumulateAndGet(value, Math::max));
    }
      ...
      try {
        for (String line : Files.readAllLines(status)) {
          if (line.startsWith("VmRSS:")) {
            String kibibytes = line.substring("VmRSS:".length()).strip().split("\\s+", 2)[0];
            return OptionalLong.of(Long.parseLong(kibibytes) * 1024L);
          }
        }
