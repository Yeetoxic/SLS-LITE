...
public final class PortableCopyRealKernelHarness {
  ...
  public static void main(String[] arguments) throws Exception {
    ...
    try (FileChannel channel =
        FileChannel.open(source, StandardOpenOption.CREATE_NEW, StandardOpenOption.WRITE)) {
    ...
    if (sparseBytes >= logicalBytes / 4) {
      throw new IllegalStateException("Sparse-aware target allocated too much: " + sparseBytes);
    }
    Files.delete(source);
    ...
    try (FileChannel channel =
        FileChannel.open(denseSource, StandardOpenOption.CREATE_NEW, StandardOpenOption.WRITE)) {
  ...
  private static long allocatedBytes(Path path) throws Exception {
    Process process =
        new ProcessBuilder("stat", "-c", "%b", path.toString())
