...
class ConfigurationValidatorTest {
    ...
    assertDoesNotThrow(
        () ->
            ConfigurationValidator.validate(
                repositories.config(), repositories.blueprints(), repositories.profiles()));
  }
  ...
  void rejectsMissingManagedLobbyBlueprint() throws Exception {
    ...
    assertThrows(
        ...
        () ->
            ConfigurationValidator.validate(
                managedLobby, repositories.blueprints(), repositories.profiles()));
  }
  ...
  void rejectsManagedLobbyAndLimboOverCombinedBudget() throws Exception {
    ...
    SLSConfig config =
        withManagedLobby(repositories.config(), "game", "test", repositories.config().limbo());
    ...
    SLSConfig config =
        withForwarding(
            ...
            new ForwardingConfig(
                ForwardingMode.MODERN, false, temporaryDirectory.resolve("forwarding.secret")));

    assertThrows(
        ConfigurationException.class,
        () ->
            ConfigurationValidator.validate(
                config, repositories.blueprints(), repositories.profiles(), true));
  }
  ...
  void allowsNoneForwardingModeWithoutOnlineModeMatch() throws Exception {
    ...
    assertDoesNotThrow(
        () ->
            ConfigurationValidator.validate(
                repositories.config(), repositories.blueprints(), repositories.profiles(), false));
  }

  private SLSConfig withManagedLobby(
      SLSConfig source, String registry, String server, SLSLimboConfig limbo) {
  ...
  private Repositories repositories(String softwareId, int blueprintMemory, int totalMemory)
      throws Exception {
  ...
  private record Repositories(
      SLSConfig config, BlueprintRepository blueprints, SoftwareProfileRepository profiles) {}
}
